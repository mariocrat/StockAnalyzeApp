...
class PolicyDocumentationTest(unittest.TestCase):
    ...
    def test_release_checklist_uses_korean_final_verification_labels(self):
        ...
        self.assertIn("OPENAI_BASIC_REVIEW_MODEL=gpt-5.6-luna", checklist)
        self.assertIn("OPENAI_BASIC_REVIEW_REASONING_EFFORT=none", checklist)
        self.assertIn("OPENAI_ADVANCED_REVIEW_MODEL=gpt-5.6-luna", checklist)
        self.assertIn("OPENAI_ADVANCED_REVIEW_REASONING_EFFORT=medium", checklist)
        self.assertIn("OPENAI_ADVANCED_REVIEW_FALLBACK_MODEL=gpt-5.4-mini", checklist)
        self.assertIn("기본값은 일반 복기 `gpt-5.6-luna`(reasoning `none`), 단일 매매 심화 복기 `gpt-5.6-luna`(reasoning `medium`)", checklist)
