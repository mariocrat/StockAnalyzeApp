...
- Confirmed nine directly defined Application tests. Callback-contract and missing OAuth configuration checks belong to Application, not the two deferred cleanup/orchestration tests. Preserve every test body and assertion; leave deferred source/stale references untouched.
- Move Application only to a gated module using existing storage_fixture, Phase A owned-HOME runtime and api_module_state without shared-helper changes. Test-local contexts restore core module namespaces/tickets, import membership/path, event-loop state and verify closed loops/tasks. Exercise actual application setup in-process for success, assertion, setup and real reload failure; check closed SQLite and removed owned directories before proceeding.
- Run only Application 9, then existing A1 minimum runner/results/SQLite regression and direct-entry gate. Stop/report a new blocker; no foundation changes, OAuth cleanup or full-suite execution. Diagnostic runner/raw bytes remain excluded and preserved. Results pending.

### Batch 4 local results

- `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B tests/run_isolated_tests.py tests.test_phase_a_application`: first execution **9 PASS / 0 failures / 0 errors**, violations/unexpected **0/0**, restored/patches_restored/cleanup **true**. Existing Application test bodies, assertions and ASGI request helpers are AST-identical; only class setup adds the local isolation base. The legacy module AST is exactly its previous form minus Application; OAuth cleanup/stale references are unchanged.
- Reuse existing runtime._OwnedCase and owned-HOME Phase A runtime, plus existing api_module_state; no helper edits. Real main/store imports and reloads, ASGI requests, SQLite and cache initialization assertions are retained. Per-case core/main namespaces, OAuth ticket contents, environment/CWD/temp/import state and event-loop policy restore in-process. Every recorded asyncio loop is closed with no remaining tasks. The original invalid-lifespan test still asserts configuration failure before cache, threads or SQLite; no successful production scheduler is started.
- Four local runTest probes exercise actual Application setup: success, assertion failure, setup failure and real main reload with invalid configuration. Exact expected failure/error counts/types are asserted, synthetic SQLite handles reject use after close, owned directories are gone before the next case, and namespace/import/environment checks pass. These probes do not add directly defined inventory testcases or run deferred OAuth tests; child exit is not used as cleanup.
- Actual production/dev/private credential/user HOME access, unintended external network/provider/subprocess/tool execution and testcase repo/worktree writes **0**, based on guarded execution and inspected paths. All data writes are synthetic testcase-owned storage/HOME/cache/temp. Coordinator Python children are test infrastructure. Foundation/A1 allowlists, production and shared helpers have no diff.
- Post-migration A1 minimum: tests.test_isolation_runner + tests.test_isolation_results + tests.test_isolation_sqlite **14 PASS**, plus direct-entry gate **1 PASS across 62 modules** (no target test bodies). Expected swallowed-violation child failures remain correctly rejected; normal negative-probe unexpected counts are zero. No Harness/Environment rerun, OAuth cleanup or full suite.
- git diff --check passed after removing trailing blank lines left by the move. Diagnostic runner/stdout.bin/stderr.bin SHA-256 unchanged; excluded from staging. Main/protected README/other worktrees untouched. D locally verified **83/85**, remaining **2 OAuth cleanup/orchestration**; Application awaits independent approval. Scoped commit authorized after these results; no push.

## H4-D batch 3 runtime migration resumed — 2026-09-19

- User accepted foundation socketpair **3 PASS** and A1 core/smoke/direct-entry **16 PASS** on the existing dirty tree at `e4e496db9c2a3219b7130c866bcb5a565f71021a`. Earlier failed attempts and blocker records remain below as history. Raw diagnosis confirmed Windows unbound getsockname WinError 10022 in the probe, not the observer call; the authorized synthetic endpoint probe passed afterwards. No further observer/helper change is authorized now.
- Authorized scope: move only PhaseAHarnessTest 5 and PhaseAEnvironmentTest 11 into a new guarded runtime test module; preserve original names/operations/assertions and leave Application 9/OAuth cleanup 2 (including stale references) unchanged. Add the new module to the direct-entry inventory. All existing dirty changes and diagnostic script/raw files remain preserved and unstaged.
- Use existing storage_fixture outside the original Phase A runtime. A test-local wrapper redirects HOME/USERPROFILE/config paths into the inner owned root after legacy environment clearing. Test-local cleanup restores env/CWD/temp/sys.path/sys.modules, package/store module namespaces, event-loop policy and guard/transport patch state. Retain real SQLite and synthetic selected-env/cache/temp operations; original negative Phase A exceptions must still be the ones asserted, without acknowledging new A1 violations.
- Add bounded success/assertion/setup-failure cleanup scenarios to the existing restoration testcase, without adding inventory testcases. Validate these 16 only, then A1 minimum regression and direct-entry gate. Stop/report any new blocker; no Application/OAuth/full suite or additional foundation redesign. Results pending. No stage/commit/push in this step.

### Runtime batch execution results

- Moved the 16 to tests/test_phase_a_runtime_environment.py with the unchanged A1 gate/storage fixture. Existing Phase A runtime remains the inner real guard; test-local wrapper only redirects HOME/USERPROFILE/APPDATA/LOCALAPPDATA and restores CWD before Windows temporary directory removal. Store dependencies import under owned storage; no main/Application/OAuth module is imported by this target. Each case restores sys.modules membership/identity, package/store namespaces, env/CWD/temp/sys.path, event-loop policy, Phase A active root/socketpair context, SQLite and requests/curl transport patch identities. Writes remain owned synthetic DB/env/cache/temp/HOME files; original repo/backend CWD and default production/dev path checks only compute paths, never open persistent stores.
- Interpreter `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B`; command `tests/run_isolated_tests.py tests.test_phase_a_runtime_environment`: **16 PASS**, first execution, failures/errors **0**, A1 violations/unexpected **0**, restored/patches_restored/cleanup **true**. Original Phase A negative audit/transport/DB/write assertions still execute and pass before A1: zero A1 ledger entries does not mean the negative checks were skipped. Real owned socketpair exchange, ASGI event loop, SQLite and selected-env/CWD behavior retained.
- Existing restoration testcase additionally runs three local Probe scenarios: success, body assertion failure and setUp failure. Each uses the same case setup, owned SQLite/cache/HOME writes and synthetic env/path/module/global changes. Expected failures/errors are asserted exactly; all cleanup checks pass, connections reject use after close and case containers are absent. No extra directly defined inventory testcase or deferred Application/OAuth case is executed.
- Actual production/dev DB/cache/private env/credential/user HOME file access **0**, unintended external network/provider/tool/subprocess **0**, testcase repo/worktree writes **0**, based on guarded execution plus inspected call paths. Internal socketpair and synthetic HOME files are intentional owned resources. The A1 coordinator's Python children are approved test infrastructure, not testcase tool execution.
- Post-migration minimum A1 runner/results/SQLite regression: **14 PASS** including expected negative child failures. Direct-entry gate: **1 PASS across 61 target modules**, no testcase bodies executed by that gate. No full suite, Application 9 or OAuth cleanup 2 execution. Historical run_phase_a_tests entrypoint selections are not updated/executed here; orchestration remains a later D concern.
- Static AST comparison against HEAD: original 16 names/bodies/assertions preserved, except the restoration method appends only the cleanup-probe call. Entire remaining legacy module AST equals the original with these two classes removed: Application/OAuth/stale references untouched. git diff --check passes. SHA-256 confirms both foundation helpers, accepted foundation probe, diagnostic runner and both diagnostic binaries are unchanged this turn; production directories have no diff.
- **D implemented/locally verified 74; remaining 11** (Application 9 + OAuth cleanup 2). Current batch awaits independent verification. Existing dirty foundation/diagnostic work and blocker records preserved, Main/protected README/other worktrees not modified. No files staged, no commit or push; commit scope and diagnostic disposition await a separate decision.

## H4-D batch 3 authorized socketpair composition plan — 2026-09-19

- Preserve the preflight blocker record below. User now permits only the shared test-helper correction needed for Phase A/A1 composition; production remains excluded. Existing unstaged plan is retained, not reset/reverted/stashed.
- Collision: both audit hooks consume one ContextVar socketpair setup. The first records the listener/connection and the second incorrectly treats that same operation as a second operation.
- Minimal correction: tests/phase_a_isolation.py tracks the exact socket/address and distinct named audit observers for each permitted bind/connect; tests/module_isolation.py supplies the A1 observer identity. Only the existing internal socketpair context creates the state; token reset stays in finally. Permit each of the two observers once for that identical owned operation, not a repeated event from one observer or another socket/address. No new address, protocol, DNS, general loopback or subprocess permission.
- First gate: focused isolated foundation probes for real owned socketpair transfer/closure with both hooks, no-context/foreign-socket/address/repeated-observer rejection, external/loopback bind/connect/DNS/datagram/HTTP and subprocess blocking, and state restoration after successful and exceptional socketpair setup. Use synthetic audit events for subprocess, never launch a tool. Then run existing A1 runner/result/SQLite regressions unchanged. Stop after two failed attempts of the same approach.
- Only after foundation passes: move Harness 5 and Environment 11 to a guarded module, preserving original operations/assertions. Add test-local owned runtime/HOME/import-state cleanup and success/assertion/setup-failure evidence; retain Application 9 and OAuth cleanup 2 unchanged/unexecuted. Run the 16 and direct-entry gate. Record exact expected violations separately from actual transport calls; no full suite or push. Scoped commit only after all gates pass.

### Foundation attempts and mandatory stop

- Implemented the proposed exact socket/address plus observer-state check in phase_a_isolation.py, with the A1 observer argument in module_isolation.py. Added test_phase_a_socketpair_isolation.py (three focused foundation testcases). Production and all D target bodies remain unchanged. This implementation is unaccepted pending successful validation.
- Ran `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B tests/run_isolated_tests.py tests.test_phase_a_socketpair_isolation` twice: first in the default sandbox, then with approved escalation to check a possible local socket restriction. Both returned tests **3**, failures **0**, errors **1**, passed **false**, violations **15**, unexpected **0**, restored/patches_restored/cleanup **true**. Recorded expected categories: 13 network and 2 subprocess. These are intentional blocked probes, not real external calls or launched tools.
- Both attempts also hit coordinator stderr decoding failure: `UnicodeDecodeError: 'utf-8' codec can't decode byte 0xc0 in position 1219`. The child result JSON remains available, but the reader thread lost the detailed testcase traceback. The underlying testcase error is not established. Source inspection identifies an unbound Windows socket getsockname() in the new pure permission probe as a likely probe defect, not a proven production or guard defect. Escalation did not resolve the failure; do not attribute it to sandbox permissions as fact.
- Per the user's two-failure rule, stopped this approach after attempt 2; no third run, foundation core regression, D 16 migration/execution, direct-entry gate, Application/OAuth/full-suite execution, commit or push. Existing blocker record below and all current scoped changes are preserved. D remaining **27**.
- Proposed materially different next step, not executed: investigate the lost child diagnostic via a byte-preserving coordinator path and replace the unbound-socket endpoint query in the pure state probe with a synthetic listener endpoint; review independently before resuming the focused gate. Do not widen network permissions or treat the 15 acknowledged denials as a passing foundation result.

## H4-D batch 3 preflight — 2026-09-19

- User independently approved all D static 58 at `e4e496db9c2a3219b7130c866bcb5a565f71021a`. Preflight confirms that HEAD, assigned test-isolation worktree, `fix/test-environment-isolation`, and initially clean full status. Batch 2 pending acceptance below is superseded; H4 remains open.
- Source inventory: PhaseAHarnessTest 5 and PhaseAEnvironmentTest 11 = **16**. Application 9 and OAuth cleanup 2 remain separate and excluded. No testcase migration or execution has occurred; stale OAuth references remain untouched.
- Pre-implementation blocker under the explicit no-A1/shared-helper-change constraint: Phase A and A1 audit hooks both call the same stateful `_socketpair_operation` from tests/phase_a_isolation.py. Phase A registers first; during its isolated_runtime, it accepts socket.bind and records `setup["listener"]`. A1 then checks the same event and rejects it because the listener is already present. The connect branch likewise consumes the shared `connected` flag. Source locations: phase_a_isolation.py:19-36,43-46,71; module_isolation.py:20,172-174,212. Read-only inspection of the configured Windows interpreter's actual socket.socketpair implementation confirms it uses the TCP fallback and calls bind/listen/connect/accept. No socket or test was executed to establish this evidence.
- Consequently the existing positive socketpair exchange (and asyncio's socketpair setup) cannot simply nest the unchanged Phase A harness under A1 while preserving the original success semantics. Treating A1 denial as success, mocking away the real socketpair, or suppressing either guard would not satisfy the authorized constraints. Shared audit/socketpair composition needs separately authorized review before this approach can proceed; no guard or allowlist was modified.
- Other required boundaries remain work to implement: own the Phase A runtime below a per-case temp root, redirect HOME/USERPROFILE/config paths after legacy env clearing, restore import/module/global state across success/assertion/setup failures, preserve original Phase A negative assertion messages. No results are claimed for those boundaries. No runtime tests, minimum regressions or direct-entry gate were run; no production/private data or external network/tool access was performed.
- Only this ExecPlan was updated with approval/inventory/blocker evidence. Main/protected README and other worktrees untouched. No commit (conditional 16 PASS requirement unmet), no push. D remaining **27** (this batch 16 plus Application 9 and OAuth 2). Resume after resolving the guard-composition scope; do not weaken the existing checks.

## H4-D batch 2 documentation/static — 2026-09-19

- User independently approved D batch 1 (26) at `ab9ad6e85dd4382a867191f50ec977ac77a05c88`. Preflight matches that HEAD, assigned test-isolation worktree, `fix/test-environment-isolation`, clean full status. This supersedes the earlier pending batch 1 acceptance; H4 remains open.
- Current-source inventory before edits: quick_verify_docs 7, owner_facing_messages 1, policy_documentation 14, render_blueprint 7, secret_scan_output 2, mobile Render D 1 = **32**. All are real repository source/document/blueprint/example reads with no unexpected runtime/tool boundary.
- Reuse unchanged A1 gate and per-case storage_fixture with env/CWD/temp/sys.path, module identity/namespace restoration and owned-container cleanup checks. Preserve every testcase name/body/assertion and real ROOT-based reads; preload the real utf-8-sig codec under A1 where used. Move only mobile Render D 1 to test_mobile_render_source_checks.py, preserving its class and method; OUT admin 1 remains in the original file, C mobile 6 unchanged.
- Scope: five static modules, new mobile D module, original mobile OUT module (only D removal/docstring), direct-entry inventory and this plan. No production/shared helper/A1/B1/B2/C or stale OAuth changes. Test writes only disposable fixture directories; target tools/subprocesses/private credentials/HOME/signing/network/provider access are not needed. Main/protected README and other worktrees remain untouched.
- Validation interpreter: `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B` (read-only runtime reuse, no install). `tests/run_isolated_tests.py` with exactly tests.test_quick_verify_docs, tests.test_owner_facing_messages, tests.test_policy_documentation, tests.test_render_blueprint, tests.test_secret_scan_output and tests.test_mobile_render_source_checks: **7 + 1 + 14 + 7 + 2 + 1 = 32 PASS**, first execution. All six reports violations/unexpected **0**, restored/patches_restored/cleanup **true**; no failed execution attempt.
- Case cleanup confirms env/CWD/temp/sys.path and module identity/namespace restoration plus removal of owned containers. Static real call-path inspection and unchanged active A1 guards support testcase repo/worktree writes **0**, target tool/subprocess execution **0**, actual private env/credential/user HOME/signing access **0**, actual external network/provider access **0**. Only synthetic fixture runtime directories are written; approved coordinator Python children are separate from testcase tools. Render URLs and secret patterns are string assertions, never live requests or scanner execution.
- Minimum A1 `-B -m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: **14 PASS**, including expected negative probes separate from batch results. Direct-entry gate `-B -m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: **1 PASS across 60 targets**, no target testcase bodies executed. No Phase A runtime 27, OUT or full-suite execution.
- Static AST comparison against starting HEAD: all **32** testcase names/complete bodies unchanged, with no normalization. Removing only Render D from the original mobile file and updating its descriptive docstring produces the exact current AST; all OUT imports/constants/class/body remain unchanged. C mobile 6 and all production/shared helper/A1/B1/B2/C/stale OAuth files unchanged. git diff --check passed; inspect full nine-file staged diff before commit.
- **D implemented/locally verified 58; remaining 27** (16 + 9 + 2). Batch 1 independently approved; batch 2 awaits independent verification. Main/protected README and other worktrees were not modified. Commit only these nine files as `test: isolate release documentation checks`; no push, automatic next batch or H4 closeout.

## H4-D inventory and release static batch — 2026-09-19

- User approved the current-source inventory at `97d01b4ccc4be096abf689e80cf54a7a4b40dc67`: **85 directly defined D testcases**, excluding subtests, nested reruns and runner-only checks. Five batches: release/setup/wrapper static **26**; quick/docs/owner/policy/Render/secret/mobile Render static **32**; Phase A harness/environment **16**; application **9**; OAuth cleanup/orchestration **2**. No C/OUT reclassification; mobile admin OUT 1 and completed C signing 8/mobile 6/favicon 1 stay separate.
- Only batch 1 is authorized: android_release_verification 13, backend_release_check_docs 4, release_env_file_setup_docs 2, release_secret_generation_docs 2, batch_wrappers 1, private_release_setup 3, android_upload_key_generation wrapper 1. Preflight: assigned test-isolation worktree, `fix/test-environment-isolation`, expected HEAD and clean full status.
- Use unchanged A1 import gate and per-case storage_fixture; assert restoration of environment/CWD/temp/sys.path, module identities and module namespace, plus owned container removal. Preserve actual repository source/example/document reads and original testcase names/assertions. Four backend docs open calls use explicit repository ROOT instead of caller CWD. No source copying or production/shared helper/A1/B1/B2/C changes.
- Test writes are confined to disposable fixture paths; no repo writes, private env/credential/HOME/keystore reads, target subprocess/npm/Gradle/keytool/secret scanner or actual external network/provider calls. Coordinator Python children are approved runner/direct-entry infrastructure. Main/protected README and other worktrees are not modified.
- Preserve known stale OAuth references in test_phase_a_environment.py: five transport methods still dispatch on pre-split OAuthLoginTest; reconcile its A1 import gate in batch 5. No correction or execution now.
- Validation interpreter: `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B`, read-only runtime reuse, no install. Ran `tests/run_isolated_tests.py` with the seven explicit batch modules above: first execution 25 passed, one Android cleanup assertion failed because the first utf-8-sig read lazily imported encodings.utf_8_sig. Original source assertions passed; violations/unexpected 0 and runner restored/patches_restored/cleanup true even in that failed module. Preload the real codec under A1 before per-case snapshots; reran only android_release_verification: **13 PASS**. Other six modules remain unchanged with their first-run **13 PASS**: final batch **26 PASS**, all seven reports violations/unexpected **0**, restored/patches_restored/cleanup **true**. No masking or assertion removal.
- Per-case cleanup checks confirm env/CWD/temp/sys.path, module identity/namespace restoration and owned directory removal. Real static call-path inspection plus active unchanged A1 write/network/subprocess guards support testcase repo/worktree writes **0**, target subprocess/tool execution **0**, real private env/credential/user HOME/keystore access **0**, actual external network/provider access **0**. Fixture directories are synthetic temporary runtime writes; approved coordinator Python child creation is not a target tool invocation. Unused legacy load_module/key-generation helpers are never called.
- Minimum regression: `-B -m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: **14 PASS**, including deliberate negative probes (their expected violations are separate from batch reports). Direct-entry gate `-B -m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: **1 PASS across 54 targets**, no target testcase bodies executed.
- Read-only AST comparison against starting HEAD: all **26** testcase names and complete bodies preserved, normalizing only the four explicit ROOT/open path adjustments. C signing module, production/shared helpers/A1/B1/B2/C and stale OAuth source remain unchanged. git diff --check passed; full scoped test diff reviewed. Initial patch construction was rejected before any write for duplicate file operations; consolidated patch succeeded. This tooling issue was separate from the disclosed codec cleanup failure.
- Nine-file commit allowlist: seven target modules, direct-entry inventory and this ExecPlan. **D implemented/locally verified 26; remaining 59** (32 + 16 + 9 + 2), independent batch acceptance pending; H4 is not closed. Authorized commit: `test: isolate release static checks`. No D batches 2–5 or full suite execution, push, or automatic continuation. Main/protected README and other worktrees were not modified.
...
- Preflight matches `9febc0bfc6901fd28d14b5956e95f3f13a9c0c6c`, assigned worktree/branch and clean full status. Main retains only its protected README unstaged at hash `e672a49f8cbf2da1d0c7492ff0e9c30b20b02a61`.
- Scope: move only favicon to tests/test_favicon_asset.py, update direct-entry inventory and this plan. Preserve the other six OUT methods, helpers and imports in test_brand_image_assets.py, without execution or reclassification. No production/shared helper/A1/B1/B2 or existing C changes.
- Read the real frontend/public/favicon.png and brand/stockboda-app-icon-light.png. Preserve RGBA conversion, (48, 48) LANCZOS resize, ImageChops.difference/getbbox assertion and favicon.svg absence check. The same test-local Pillow isolation pattern as the approved icon batch owns copied registries and fresh mode cache; all source/converted/resized/difference images close before cache/registry restoration. Per-case storage_fixture restores env/CWD/temp and removes owned runtime directories. No asset generation, source copy, tool execution or external requests.
- Static comparison against starting HEAD: removing only the favicon method yields the exact current AST of test_brand_image_assets.py. All six OUT methods, helper functions, imports and constants are unchanged. New module directly defines exactly the original favicon testcase name; pixel operations, inputs and both assertion meanings are preserved, with explicit ownership added for six image objects.
- `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B tests/run_isolated_tests.py tests.test_favicon_asset`: **1 PASS**, first execution. Violations/unexpected **0**, restored/patches_restored/cleanup **true**. Cleanup verifies all six images are closed, original Pillow registry identities/contents, initialization flag and mode-cache identity/statistics are restored, env/CWD/temp match baseline and the owned case container is absent. Testcase repo/worktree writes **0**, asset changes **0**, actual external network/provider access **0**, based on inspected real read/compute call paths and unchanged active A1 write/network/subprocess guards.
- Minimum A1 `-B -m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: **14 PASS**, including intentional negative probes. Direct-entry gate `-B -m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: **PASS**, 47 guarded targets, no testcase bodies executed. Static OUT preservation and `git diff --check`: PASS. No failed execution attempt.
- Four scoped files: new favicon test, original brand test with only favicon removed, direct-entry inventory and this plan. Production/shared-helper/A1/B1/B2 code and existing C tests remain unchanged. Main status/protected README hash match preflight; other worktrees untouched. **C implemented/locally verified 23, remaining 0**; original 22 independently approved, favicon 1 awaits independent verification. OUT/D/full suite unexecuted. Ready for authorized commit `test: isolate favicon asset check`, then wait. No push or automatic C/H4 closeout.

## H4-C signing batch — 2026-09-18

- Mobile C 6 independently verified and approved by the user at `e991c36761535743fefee9b77edb831dbdba81f3`. Preflight matches that HEAD, assigned branch/worktree and clean full status. Main retains only its protected README unstaged, hash `e672a49f8cbf2da1d0c7492ff0e9c30b20b02a61`.
- Scope: C signing 8 separated into `tests/test_android_signing_checks.py`, existing direct-entry inventory and this plan. D wrapper remains in its original file/class with unchanged method body, unexecuted. Source reinspection confirms no new tool/private boundary: only the stale-path exists() probes an external path; other legacy signing/tool paths are string values only. No production/shared-helper/A1/B1/B2 changes.
- Reuse import gate and per-case storage_fixture. Disable bytecode before dynamically loading the real script; fresh modules are not registered globally. Restore sys.path/sys.modules/bytecode and env/CWD/temp; verify restoration and owned-container removal after cleanup. Test-local spies reject main/create_upload_key/find_keytool/_repo_root and assert zero calls. A test-local exists wrapper delegates to real Path.exists only for testcase-owned paths, covering stat probes absent from the A1 audit.
- Stale fixture uses synthetic temp old/current workspace paths with the same release-private/android/alphamate-upload.jks suffix; real production existence/replacement logic remains unchanged. Template/custom paths remain literal contract inputs and are never opened. No real HOME/private env/credential/keystore access, key generation or testcase child process is authorized. Only synthetic env files and owned temp directories may be written.
- Static AST comparison: 9 original cases = C 8 + D 1, no loss/duplicates; every original assertion is retained. D wrapper method text is identical and remains in the original class with its imports/helper untouched. C changes only fixture/module acquisition and the stale path input plus its owned missing-path precondition.
- `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B tests/run_isolated_tests.py tests.test_android_signing_checks`: **8 PASS**, first execution. Violations/unexpected **0**; restored/patches_restored/cleanup **true**. No testcase child process/keytool execution and main/create_upload_key/find_keytool/_repo_root spies all uncalled. Private env/credential/real HOME/keystore access **0**, external network/provider **0**, testcase repo/worktree writes **0**, based on reviewed call paths, owned exists enforcement and unchanged A1 open/write/network/subprocess guards. Dynamic loader checks bytecode disabled before exec_module; A1 also enforces no repository writes, including scripts/__pycache__. Synthetic .env.release content is the only signing configuration read/written; no keystore is created.
- Bounded in-memory cleanup probe under A1: **3 scenarios PASS** (success, body assertion after real synthetic env writes, setup failure after actual dynamic import). Deliberate env/CWD/sys.path/sys.modules/temp/module mutations restore; Path.exists binding restores, case containers disappear, and weak references confirm fresh helper modules are collectible after cleanup. Violations **0**, restored/patches_restored/cleanup **true**. No probe file persisted or extra signing/D testcase body executed.
- Minimum A1 `-B -m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: **14 PASS**, intentional negative probes retained. Direct-entry gate `-B -m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: **PASS**, 46 guarded targets, no testcase bodies executed. `git diff --check`: PASS; no failed execution attempt.
- Four scoped files: new C signing module, original file retaining D wrapper, existing direct-entry inventory and this plan. Production/shared helpers/A1/B1/B2 unchanged. Main status/README hash match preflight; other worktrees untouched. **C implementation remaining 0 (8 assets + 6 mobile independently approved; 8 signing locally verified, independent verification pending)**. D wrapper/full suite unexecuted. Ready for authorized commit `test: isolate android signing checks`, then wait. No push, automatic C closeout or H4 completion claim.

## H4-C mobile source batch — 2026-09-18

- First C asset batch (8) independently verified and approved by the user at `23a944f0d3d0396ed577caa9bebe2cc8a6155fe0`. Preflight confirms that HEAD, the assigned worktree and branch, and a clean full status. Main retains only its protected README unstaged at hash `e672a49f8cbf2da1d0c7492ff0e9c30b20b02a61`.
- Scope: only the six approved mobile source/static cases, their separation from the existing mixed module, existing direct-entry inventory and this plan. Move C method bodies verbatim into `tests/test_mobile_source_checks.py`; keep D Render and OUT admin-source in their original module/class, with unchanged bodies/classification and no execution. No runner/shared-helper/A1/B1/B2/production changes.
- New C module imports only the existing guard/fixture and standard library; no pandas/FastAPI, backend application import or import of the deferred module. Real JSX/CSS/SVG/backend source reads and all original assertions remain. Import gate precedes discovery; per-case storage_fixture restores env/CWD/temp, followed by checks of module namespace, sys.modules identities, sys.path and owned-container removal. No testcase subprocess, Node/npm/Vite, provider/network or repo writes. Only the existing runner/direct-entry regression infrastructure launches Python children.
- Static AST/source-segment comparison against starting HEAD: all original 8 method bodies are text-identical across C 6 and deferred D/OUT 2, with no duplicate or missing testcase. D/OUT module/class/imports and their two methods are retained; only the C methods moved and the module description was updated. No deferred-module import or testcase execution in this batch.
- `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B tests/run_isolated_tests.py tests.test_mobile_source_checks`: **6 PASS** on first execution; violations/unexpected **0**, restored/patches_restored/cleanup **true**. Per-case env/CWD/temp/sys.path, sys.modules membership/identity and test-module namespace identity checks passed after fixture cleanup; owned case containers were absent. Source is read as UTF-8 text, never imported as an application. Actual external network/provider access **0**, testcase repo writes **0**, based on inspected call paths plus the unchanged active A1 write/network/subprocess guards. No Node/npm/Vite or testcase child process ran; no build/dist/asset changes.
- Minimum A1 regression `-B -m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: **14 PASS**, with intentional negative probes retained. Direct-entry gate `-B -m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: **PASS**, 45 guarded targets, no testcase bodies executed. `git diff --check`: PASS. No failed execution attempt.
- Four scoped files: new C module, original mixed module retaining D/OUT, direct-entry inventory and this plan. Production/shared-helper/A1/B1/B2 code is unchanged. Main status and protected README hash match preflight; other worktrees untouched. **C remaining 8 (signing)**, unexecuted; full suite unexecuted. Ready for authorized commit `test: isolate mobile source checks`, then wait; no push or C/H4 closeout. Mobile independent verification remains pending.

## H4-C inventory and asset batch — 2026-09-18

- User approved the read-only inventory at `396d9cac3ffc1dd534a8c6266b09526e4764e0b5`. C is **22**: ads 3 + splash 2 + icon 3; mobile 6; signing 8. Three implementation batches in that order. The signing file actually contains 9 cases: its batch-wrapper source contract belongs to D. Mobile retains the existing Render D 1 and admin-source OUT 1; do not execute those through C discovery.
- Current authorized scope: the three asset test modules (8 cases), existing direct-entry inventory, and this plan. Preflight confirms the assigned worktree, branch `fix/test-environment-isolation`, expected HEAD and clean full status. Main has only its protected README unstaged, hash `e672a49f8cbf2da1d0c7492ff0e9c30b20b02a61`; preserve it and all other worktrees.
- Preserve real production JSX/CSS/PNG/XML reads and original assertions. No source copying, asset generation, Node/npm/Vite/keytool, build/dist changes or provider calls. Reuse the unchanged A1 import gate and per-case storage_fixture; writes are confined to synthetic owned temporary roots. No production/shared helper/A1/B1/B2 changes or new cache/HOME permissions.
- Icon path enumeration moves from module import to per-case setup. Built-in Pillow plugins initialize under A1 before cases; each case owns copied registries and a fresh cache around the real ImageMode calculation. Source handles and converted/channel images close explicitly. Cleanup asserts image closure, original registry identities/contents, initialization flag and original cache identity/statistics, followed by env/CWD/temp restoration and removal of the owned case container.
- Remaining inventory boundaries: mobile 6 is source/static (including one mixed frontend/backend logging contract), with unnecessary pandas/FastAPI imports to address in its own batch. Signing 8 uses Python helpers, synthetic temp env files and command/string construction, not keytool execution; its stale-workspace case currently probes an external `D:/old/.../release-private/android/alphamate-upload.jks` with exists(). Move that fixture path inside owned temp while preserving real existence semantics in the later batch. None of the C cases requires Node/npm/Vite, tool caches, user HOME, real signing material or external network permissions. Static/source assets remain real repository reads; no temp copy is needed for this batch, only the existing fixture's owned runtime directories.
- Validation: `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B tests/run_isolated_tests.py tests.test_app_ad_display_wiring tests.test_app_splash_ui tests.test_android_icon_alignment`: **3 + 2 + 3 = 8 PASS**, first target execution. Every module reports violations/unexpected **0**, restored/patches_restored/cleanup **true**. Actual external network/provider access **0** and testcase repo writes **0**, based on inspected call paths and unchanged active A1 network/subprocess/write guards. No build/dist/Android asset changes. Real image decoding and the original alignment tolerance, opacity, background color, pixel-color predicates and source/static assertions are preserved.
- Minimum regression: `-B -m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: **14 PASS**, including expected negative boundary probes. Direct-entry gate `-B -m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: **PASS**, 44 targets, no target testcase bodies executed.
- Focused in-memory icon cleanup regression under A1: success, synthetic body assertion and partial setup failure all restore registry identity/content, mode-cache identity/statistics, initialization flag, PIL module bindings, env/CWD/temp and path attributes; opened images are closed and owned case containers removed. **3 scenarios PASS**, violations **0**, restored/patches_restored/cleanup **true**. The first probe's three case cleanups passed but its final coordinator comparison failed because TemporaryDirectory initialized an initially unset tempfile cache after baseline capture. Corrected only the transient probe to resolve coordinator temp before baseline; rerun passed. No product/test implementation failure or guard relaxation; no probe file persisted.
- Full diff reviewed; `git diff --check` passed. Changed paths are only the three target modules, existing direct-entry inventory and this plan. Production/shared helper/A1/B1/B2 code remains unchanged. Main's only change remains its protected README at the original hash; other worktrees are untouched. **C remaining 14** (mobile 6 + signing 8), unexecuted; full suite unexecuted. Ready for the authorized scoped commit `test: isolate frontend asset checks`, then wait. No push or C/H4 closeout.
...
- Source boundary confirmed: two synchronous fake executors/completed Futures (executor/results contract only); two real ThreadPoolExecutor calculations with FIRST_COMPLETED and thread-local fake sessions. Production catches ordinary provider exceptions and returns empty rows; distinguish this from a propagating Future worker exception. No lifespan/asyncio or additional worker boundary.
- Scope: Theme target, existing direct-entry inventory and this plan. Preserve original names, data and assertions; reuse storage_fixture/_import_state and restore data_fetcher dictionaries. Test-local instrumentation uses real executors and checks thread termination before session/provider patches exit, followed by module/env/temp restoration; fault subtests cover assertion, worker/provider and partial submit/setup failures. No production/shared helper/A1/B1 changes, full suite, other worktree changes, push or automatic closeout. Validation writes only synthetic owned temp storage; two same-approach failures require stopping.
- Fake executor cases keep their completed-Future execution and original bound/exception assertions; added weak-reference checks confirm Future/result references are released. This is an executor/results cleanup contract, not evidence of real worker joins.
- Both real cases retain original inputs, calculations and assertions, with a real ThreadPoolExecutor subclass observing submissions/shutdown and a wrapping wait preserving FIRST_COMPLETED. The session wrapper delegates to the original fake session and records weak references/thread ownership. Executor exit and an explicit cleanup shutdown check every owned thread is dead and every Future complete before the active session/provider patches restore. No worker is delegated to child-exit cleanup.
- Each real testcase additionally runs four cleanup subtests: assertion during wait while a session is gated, a Future worker exception after fetching, a provider exception intentionally converted to empty results by production, and failure on the second submit while the first session is gated. Exact synthetic exceptions are distinguished from unrelated failures; cleanup releases the gate before shutdown. Patches are checked restored while owned storage still exists, then env/CWD/temp/module/thread state matches baseline and nested roots are gone. Testcase cleanup confirms all monitored session wrappers and underlying fake sessions are collectible before data_fetcher/env/temp restoration, including sessions kept in the original local list until the method returns.
- Interpreter `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B`, no installation. `tests/run_isolated_tests.py tests.test_theme_cache_workers`: **4 PASS**, first execution, including all eight real-case cleanup subtests. Violations/unexpected **0**, actual external network/provider access **0** under unchanged guards, restored/patches_restored/cleanup **true**. Surviving real executor worker/thread **0**. Original four testcase names, input literals and assertions preserved by full diff review.
- Minimum A1 regression `-m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: **14 PASS** (intentional negative probes retained). Direct-entry gate `-m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: **PASS**, 41 guarded targets, no target testcase execution. `git diff --check`: PASS. No failed test attempt.
- Main remains modified only at the protected README, hash `e672a49f8cbf2da1d0c7492ff0e9c30b20b02a61`, unchanged. Only the three scoped files changed; no production/shared-helper/A1/B1 edits or other worktree changes. **B2 implementation remaining 0** (2 Auth + 1 Review independently approved; 4 Theme locally verified, independent approval pending). H4 incomplete; B2 closeout is not performed. Ready for authorized commit, then wait; no push.

## H4-B2 review concurrency batch — 2026-09-17

- User independently approved Auth lifespan 2 at `6b2e785295c772cd6e6ecd6cf015cc4042855926`. Preflight confirms that HEAD, assigned worktree, `fix/test-environment-isolation`, and clean full status.
- Only `test_review_example_trades_are_seeded_once_when_logins_overlap` is authorized: real two-worker executor, Barrier/Events, main lock and owned SQLite; no lifespan, asyncio or unexpected worker boundary. Theme 4 remains deferred.
- Scope: target module, existing direct-entry inventory and this plan. Reuse storage_fixture/api_module_state, snapshot both backend.core aliases reloaded by the legacy helper, preserve seeding and original assertions. Verify success/assertion/worker-error/partial-setup cleanup inside the one testcase: release event and abort barrier, shutdown and inspect every executor thread/future while patches/storage are live, then restore patches, module/environment and owned storage. No production/shared helper/A1 guard/B1 isolation edits.
- Read-only inspection; selected tests and minimum regressions may create/remove synthetic owned temporary storage. No external provider, other worktree writes, Theme/full-suite execution or push. Two same-approach failures require stopping.
- Implementation keeps the original testcase name and successful SQLite count/summed-result assertions. Four subtests exercise success, an exact synthetic assertion before release, an exact exception propagated from one worker while its peer is in seeding, and partial executor setup failure with one worker waiting for its unsubmitted peer. The three full scenarios prove two distinct real worker threads; partial setup proves its one created worker is released through BrokenBarrierError. No extra discovered testcase or unrelated suite is executed. Bounded wait limits are now 5 seconds; event expiry raises instead of silently permitting seeding.
- Cleanup explicitly sets release, aborts the barrier, calls shutdown(wait=True), checks every executor-owned thread is not alive and every future is done, and checks expected worker outcomes while patches and owned storage/env remain active. A later patch-stack callback verifies both original callables are restored while storage still exists. Finally each subtest verifies root removal, original main lock identity/state, all core store/main and backend.core account/access bindings, env/CWD/temp/sys.path, logger filters/level and thread baseline. Child exit is not the cleanup mechanism.
- Validation interpreter: `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B`, no installation. `tests/run_isolated_tests.py tests.test_review_access_concurrency`: **1 PASS**, all four subtests, first execution; violations/unexpected **0**, restored/patches_restored/cleanup **true**. Surviving worker/thread **0** in every subtest. Actual external network/provider calls **0**, under unchanged A1/B1 guards; no denied network operation consumed as a synthetic failure.
- Minimum A1 regression `-m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: **14 PASS**, including intentional negative probes. Direct-entry gate `-m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: **PASS**, 40 guarded targets, no target testcase executed. `git diff --check`: PASS; full source diff reviewed. No failed test attempt.
- Main status remains only the protected README modification; hash `e672a49f8cbf2da1d0c7492ff0e9c30b20b02a61` unchanged. Scope remains three files; production/shared helpers/A1/B1 unchanged. Ready for authorized commit and independent user review, not automatic closeout. B2 remaining **4 Theme tests**, unexecuted; H4 incomplete, no push or next batch.

## H4-B2 Auth lifespan batch — 2026-09-17

- Approved B2 source inventory: 7 tests, in three batches: Auth lifespan 2, review concurrency 1, theme workers 4. Auth uses real asyncio/lifespan with non-running FakeThread; review uses a real two-worker executor; theme has two fake-executor and two real-executor cases. No C/D/OUT reclassification. Only Auth is authorized now.
- Preflight: assigned test-isolation worktree, branch `fix/test-environment-isolation`, HEAD `dd4067ee07aadb11b7b8f8b4cc541a2e8b433114`, clean staged/unstaged/untracked state.
- Scope: Auth target, existing direct-entry inventory and this plan. Reuse storage_fixture/api_module_state; preserve real startup configuration validation, yfinance cache initialization, logging installation, asyncio.run and shutdown. Keep existing non-running FakeThread. Check scheduler stop event, closed loop/no pending tasks, no surviving threads, and restored environment/module/logger/yfinance state before child exit. Production join behavior is outside scope.
- Inspection is read-only; selected tests/regressions create/remove only synthetic owned temporary storage. A1 guard/allowlist, B1 network isolation, production/shared helpers and Main protected README remain unchanged. No review/theme/full-suite execution or push. Same-cause two-failure stop applies.
- Implementation: the two original names and warmup/FakeThread assertions remain. A module import gate precedes dependencies; per-case storage_fixture and api_module_state restore all five store/main dictionaries, environment, CWD/temp, sys.path and logging filters. Real startup functions are observed with wrapping spies, not disabled. The actual scheduler closure supplies its real stop Event without invoking its target. A separate asyncio policy protects the caller's loop state.
- Installed yfinance source sets timezone, cookie and ISIN cache locations and closes pre-existing manager DB handles. Test-local attribute contexts preserve all three locations/handles without exposing baseline handles to the setter; real initialization creates only the owned cache directory. Per-case post-cleanup assertions compare cache singleton identities, module/global bindings, logger/filter identities and contents, thread identities, policy and environment against baseline before the child exits. Both loops close with no pending tasks; stop Events change from unset inside lifespan to set after shutdown. No actual scheduler/warmup worker starts.
- Validation interpreter: `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B` (read-only reuse, no installation). `tests/run_isolated_tests.py tests.test_auth_lifespan`: **2 PASS**, first execution, violations/unexpected **0**, restored/patches_restored/cleanup **true**. Both cases pass loop/stop/cache/startup-spy and post-cleanup state assertions; owned per-case roots are removed. Actual external network/provider calls **0** with unchanged A1 guards and original non-running thread doubles.
- Minimum A1 regression: `-m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: **14 PASS**; intentional negative probes retain their expected violations/failures. Direct-entry gate `-m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: **PASS**, 39 targets. No target testcase executes in that import gate. `git diff --check`: PASS. Static full-diff review confirms only the three scoped files changed, no production/shared-helper/A1/B1 edits.
- Main has only its protected README modification, blob `e672a49f8cbf2da1d0c7492ff0e9c30b20b02a61`, unchanged. No other worktree modified. Review concurrency 1 and theme workers 4 remain unexecuted: B2 remaining **5**, H4 incomplete. Auth batch ready for the authorized scoped commit and user review; no independent closeout approval claimed, no push or automatic next batch.
...
- Approved four-batch plan: (1) purchase/credential failure/refund/reconciliation 12, (2) subscription/acknowledge/RTDN/OIDC 15, (3) AdMob SSV/reward 8, (4) chart/market + AI finalization 3. Only batch 1 is authorized now; after its completion 26 remain. No automatic next batch.
- Source confirms exactly 12 below, split from test_billing_provider_verification into test_billing_purchase_transport. Remaining 23 subscription/AdMob bodies and original helpers stay unchanged and unexecuted. No additional worker/provider boundary found in this batch.
- Credential failure: test_google_play_purchase_does_not_grant_when_verification_cannot_run keeps synthetic RSA service-account construction and actual Credentials.refresh/JWT signing. A testcase-owned patch on google.auth.transport.requests.Request.__call__ raises a specific synthetic TransportError before requests dispatch. Added assertions require that exact cause, one token POST to oauth2.googleapis.com/token, form content-type and JWT-bearer grant/assertion; original HTTP 503/no-credit assertions retained. A1 denial cannot substitute for this expected failure; guard/allowlist unchanged.
- Purchase/consume six: test_verified_google_play_consumable_grants_credits_once, test_google_play_consumable_rejects_non_processed_order, test_google_play_purchase_verification_loads_actual_order_amount, test_google_play_purchase_stored_fields_are_length_limited, test_google_play_consumable_consume_failure_can_be_retried_without_duplicate_credits, test_google_play_purchase_rejects_wrong_product. Original _verify_google_play_purchase and _consume_google_play_product fakes retained independently. Amount verification retains _google_play_headers and requests.get HTTP fakes through real purchase/order JSON parsing. Consume false/true retry and duplicate-credit prevention remain real local application logic.
- Refund/reconciliation five: test_google_play_full_refund_revokes_only_the_matching_order_once, test_google_play_canceled_order_revokes_remaining_credit_without_refund_amount, test_failed_review_refund_does_not_restore_credit_to_refunded_order, test_google_play_order_verification_reads_full_refund_details, test_voided_purchase_reconciliation_handles_chargebacks_partial_refunds_and_item_failures. Original order-function fake or headers/requests.get fake retained at each seam; real local orders, balances, receipt idempotency, usage reversal, refund/cancel locks, partial-refund and per-item failure handling remain intact. Voided request params and verify_order.assert_not_called preserved.
- HTTP boundary source: product-token GET and orders GET use authorization headers and timeout 10; consume POST is replaced only where the original tests already replaced the production function. Voided listing retains its original HTTP fake. No new transport coverage is claimed beyond original assertions plus explicit credential failure checks.
- Reuse require_storage_boundary, storage_fixture and _import_state; snapshot backend access_control dictionary before reload so body-local reloads/direct assignments restore. Original HTTP patch contexts restore requests.get; credential patch registered with enterContext. Five DB paths/cache/config/temp/CWD/env are owned and restored per case. No DB operation is replaced by a mock, and original closing/finally blocks are preserved.
- Scope: new purchase test module, original provider split module, existing direct-entry target list and this plan only. Inspection/AST/Git checks read-only; selected runner/regression/state probes create/remove synthetic owned storage. Production/A1/shared helpers/Main/README untouched; no full suite, next batch or push. Same-cause two-failure stop applies. Validation pending below.

### B1 purchase batch 1 validation and handoff

- Executed with `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe -B`; no package installation. `tests/run_isolated_tests.py tests.test_billing_purchase_transport`: **12 PASS**, first attempt; violations/unexpected **0**, restored/patches_restored/cleanup **true**. Actual Google/Play/OAuth/external network **0**: unchanged A1 guards remained installed, no network violation/access attempt reached them in the target run. Credential transport fake was invoked exactly once and the original 503 was caused by that exact synthetic TransportError, not an A1 denial or unrelated configuration error.
- AST proof against starting HEAD: 35 = 12 selected + 23 deferred, disjoint exact name union. Eleven selected and all 23 deferred entire testcase bodies are identical. Removing only the added synthetic credential seam/checks reproduces that testcase's entire original AST; original assertions/inputs/expected values remain. Both original helper ASTs identical. No skip or fake consolidation.
- State probe under unchanged isolated_module/protected_paths: all 12 cases individually PASS. After each, dictionary keys/value identities match for backend/core access_control, core account_store, requests, time and urllib.request; env/sys.path/CWD/tempfile/temp contents/logger state restored. Google Request.__call__, Credentials.refresh and A1 requests Session.request identities restored. Injected assertion failures after the credential-failure and consume-retry bodies, plus setup failure immediately after reload, also restore state. No target/deferred provider import crossover, violations 0, boundary patches restored and container cleanup true. Probe runs only in memory; no shared framework changes.
- Ledger/refund evidence is local SQLite, not mocked persistence: duplicate grants, consume-pending/completed retry, encrypted purchase token/order amounts, order-specific full refund and receipt idempotency, cancellation without refund money, reversed usage without restoring refunded credits, and voided chargeback/partial-refund/item-failure states all retain their original assertions and PASS.
- A1 minimum `-m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: **14 PASS**, including intentional negative probes (separate from target violations). `tests/run_isolated_tests.py tests.isolation_smoke tests.test_rate_limit`: **1 + 3 PASS**, unexpected 0, restored/patches_restored/cleanup true.
- Direct-entry gate `-m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: PASS across **34** targets; added only purchase transport. Full suite and batches 2-4 were not executed. No failed implementation/probe attempt.
- `git diff --check` PASS; explicit four-file staged-name/full-diff/whitespace review before commit `test: isolate billing purchase transport tests`. Production/A1 guard/allowlist/shared helpers changed: no. Main protected README status/hash rechecked before and after commit; no other worktree changes or push.
- Remaining B1 **26** = subscription/acknowledge/RTDN/OIDC 15 + AdMob 8 + chart/market/AI finalization 3. Batch 1 ready for independent verification after commit; no independent approval of this batch or overall H4 completion is claimed. Wait for user direction.

## H4-B1 OpenAI transport start — 2026-09-16

- OAuth B1 five-test migration independently verified and approved by the user at `03a86dfdde1574886a4946ed469c108493576a9c`. Preflight: that HEAD, `fix/test-environment-isolation`, assigned test-isolation root/worktree and clean full Git status. Main retains only its protected README modification, blob `e672a49f8cbf2da1d0c7492ff0e9c30b20b02a61`.
- Current source reclassification: 11 = 10 OpenAI transport + 1 deferred chart-provider test. `test_advanced_review_override_can_disable_fallback_for_qa_comparison` invokes build_advanced_ai_review -> _compact_chart_snapshot/build_journal_charts, then _contexts_for_trades/get_stock_ohlcv if needed, before its fake _call_openai_review. Additional chart/market isolation is required. Move it intact with original setup/teardown/response helper to test_ai_review_chart_provider.py; do not import or execute it. No extra provider fake or forced eleven-test PASS.
- Transport inventory (all retain their complete original bodies):
  - test_openai_review_retries_transient_rate_limit_once: urlopen raises HTTPError 429 once, then synthetic JSON; two attempts, backoff 0, retry-ok.
  - test_openai_review_timeout_is_configurable: urlopen receives timeout 12, timeout-ok.
  - test_openai_review_applies_review_specific_output_limits: JSON request max_output_tokens [1000, 3000].
  - test_openai_review_sends_review_specific_reasoning_effort: request model/effort/output tuples preserve basic none/1000 and advanced medium/3000.
  - test_openai_review_reasoning_effort_override_and_invalid_value_are_safe: request effort [high, medium].
  - test_openai_review_disables_response_storage: request store false and synthetic private-ok response.
  - test_openai_review_output_limits_are_safely_bounded: request output limits [256, 10000].
  - test_openai_review_records_token_usage_without_prompt_content: urlopen JSON usage parsing and original record_event lambda; usage/privacy assertions unchanged.
  - test_openai_review_runtime_settings_have_upper_bounds: repeated HTTPError 429 ends in RuntimeError; four calls at timeout 90, three captured sleep calls of 5.0 seconds.
  - test_openai_review_reads_api_key_from_explicit_env_file: owned synthetic env file read and Authorization Bearer sk-env-file assertion; original unlink finally retained.
- Actual production boundary: _call_openai_review constructs urllib Request POST https://api.openai.com/v1/responses with authorization/content-type and JSON options, calls urlopen and parses UTF-8 JSON. Existing success and 429 retry/exhaustion cases are preserved. URLError/TimeoutError and malformed JSON branches exist but are not directly exercised by these ten original cases; no new coverage claim for them.
- Isolation: reuse require_storage_boundary, storage_fixture and _import_state. Snapshot ai_review_v2 dictionary before reload. Register patch.object with original urllib.request.urlopen and time.sleep before the unchanged bodies assign their existing fakes; LIFO cleanup restores transport/time before module/import/storage even on failure. Module snapshot restores record_event. No A1 guard/allowlist, shared helper or production changes.
- Env/file: sanitized testcase owns all five DB paths, cache/temp/config/CWD. Explicit env-file test uses only its original synthetic key file under owned temp; no host credentials or private env read. Failure-event DB writes remain real local guarded writes rather than being mocked away. No app lifespan or provider background work is invoked.
- Scope allowlist: transport module, new deferred chart-provider module, existing direct-entry target list and this ExecPlan. Read-only inspection/AST/Git commands; selected runners and state probes may create/remove only synthetic owned temp storage. No full suite, Billing/other B1 execution, production changes or push. Two same-cause failures stop the approach.
- Validation follows below. After the ten-test migration, remaining B1 is 38 (48 - 10), including the deferred chart-provider testcase. Independent verification is still required.

### B1 OpenAI validation and handoff

- Interpreter `D:/Project/Vibe/StockBoda/.venv/Scripts/python.exe`, always `-B`; no install. `-B tests/run_isolated_tests.py tests.test_ai_review_openai_transport`: 10 PASS on first invocation; violations 0, unexpected 0, restored/patches_restored/cleanup true. No actual OpenAI/external network call or access attempt: unchanged A1 network guards stayed active throughout. No failed implementation/test attempt in this batch.
- Full AST comparison against starting HEAD: exact eleven-test name union, disjoint 10 + 1 split, complete testcase bodies identical; original response helper identical in both modules and deferred setup/teardown identical. No skips or weakened assertions. Existing model/options/header/response/usage inputs and expectations preserved. Retry success retains two calls; retry exhaustion retains four calls at timeout 90 and three sleep values 5.0; configurable timeout 12 retained.
- Additional in-memory state probe used unchanged isolated_module with protected_paths. Each of ten cases ran individually, including 429 retry success and retry exhaustion. After every case, full dictionary keys/value identities matched for ai_review_v2, urllib.request, time, requests and core.event_log; env, sys.path, CWD, tempfile.tempdir, temp-directory contents, uvicorn.error filters and yfinance logger level matched baseline. Synthetic assertion failure after the original retry-exhaustion body plus a record_event replacement, and synthetic setup failure immediately after reload, also restored state. Boundary patches restored; container removed; violations 0. Deferred chart-provider module was never imported. Probe source ran in memory, adding no framework/test file.
- A1 minimum `-B -m unittest tests.test_isolation_runner tests.test_isolation_results tests.test_isolation_sqlite -v`: 14 PASS including intentional negative probes (their denied network attempts are separate from the OpenAI target's zero violations). `-B tests/run_isolated_tests.py tests.isolation_smoke tests.test_rate_limit`: 1 + 3 PASS; unexpected 0, restored/patches_restored/cleanup true.
- Direct gate `-B -m unittest tests.test_storage_fixture_runner.StorageFixtureRunnerTest.test_direct_import_fails_before_storage_dependencies_or_files -v`: PASS across 33 targets. Only the OpenAI transport module was added; deferred chart-provider module not executed. No full suite or other B1 provider execution.
- git diff --check PASS. Production, A1 guard/runner/allowlist, shared helper and README changes: none. Four-file allowlist/full staged diff check precedes the authorized commit `test: isolate openai transport tests`; Main status and protected README hash are rechecked at handoff. No push or other worktree write.
- Remaining B1: **38**, including **1 deferred chart-provider testcase** from this source inventory. Final verdict for the ten OpenAI transport tests: ready to request independent verification. Eleven-test overall completion is not claimed; the chart-provider boundary requires a separately scoped follow-up, and Billing/other B1 work remains unexecuted.
