...
광고 보상 정책 안전 상한: 심화 복기권 1장당 보상형 광고 최대 20회.
...
| 상품 | 가격 | 용도 |
| --- | ---: | --- |
| 일반 복기 15회권 | 2,900원 | 구독 없이 일반 복기 추가 사용 |
| 일반 복기 25회권 | 4,500원 | 일반 복기를 자주 쓰는 사용자 |
| 심화 복기권 10회 | 3,900원 | 심화 복기 추가 사용 |
| 심화 복기권 20회 | 6,900원 | 심화 복기를 자주 쓰는 사용자 |
| Pro 월 구독 | 출시 7,900원, 정상 9,900원 | 결제 주기마다 일반 35회, 심화 25회와 광고 제거 |

Pro는 Google Play Console에 `pro_monthly` 구독 상품 하나로 구성한다.

- [ ] 정상 월간 기본 요금제를 9,900원으로 만든다.
- [ ] 공개 출시일로부터 3개월 동안 신규 구독할 수 있는 출시 혜택을 만든다.
- [ ] 출시 혜택 가격은 첫 3회 월 결제까지 7,900원으로 설정한다.
- [ ] 네 번째 결제부터 정상 가격 9,900원이 적용되는지 확인한다.
- [ ] 내부 테스트와 비공개 테스트 기간은 출시 이벤트 3개월에 포함하지 않는다.
- [ ] 이벤트 종료 전에 가입한 사용자의 남은 할인 결제 횟수가 유지되는지 확인한다.
...
VITE_ADMOB_APP_OPEN_AD_UNIT_ID=ca-app-pub-...
...
- [ ] 앱을 90초 이상 백그라운드에 둔 뒤 돌아올 때 앱 오프닝 광고가 노출될 수 있는지 확인한다.
