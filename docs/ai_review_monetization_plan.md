...
- 심층 복기: 이번 매매와 최근 매매를 비교하는 깊은 AI 복기. 내부 타입은 `advanced`.
...
보상형 광고 정책은 서버 설정으로 조정한다. 기본값은 광고 1회당 일반 복기 1회, 광고 5회당 주간 심층 복기권 1장이다. `ALPHAMATE_ADS_PER_ADVANCED_TICKET`으로 심층 복기권 지급 기준을 바꿀 수 있다. `ALPHAMATE_FORCE_REWARDED_AD_CHAIN` 플래그도 있지만 기본값은 `false`다. 여러 광고를 연속으로 강제 시청시키는 흐름은 사용자 피로도가 높기 때문이다.
