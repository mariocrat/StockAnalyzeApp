...
- `POST /api/auth/dev-login`: 개발 중 카카오/네이버 로그인 없이 같은 DB 구조를 테스트하기 위한 임시 로그인
...
- `accounts.sqlite3`에 사용자, 로그인 제공자 연결, 세션을 저장한다.
- `POST /api/auth/dev-login`으로 카카오/네이버 계정처럼 보이는 개발 사용자를 만든다.
- 기존 복기권 시스템이 AlphaMate 세션 토큰을 받아 사용자별 이용권을 분리한다.
- 이 단계는 실제 카카오/네이버 키 없이 가능한 선행 작업이다.
